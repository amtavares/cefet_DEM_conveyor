...
class Prepare:
    ...
    def preparar_transportador(self, config):
        ...
        x1_correia = 0
        y1_correia = 0
        x2_correia = comprimento_total_correia * math.cos(angulo_real_correia)
        y2_correia = comprimento_total_correia * math.sin(angulo_real_correia)
        posicoes_correia = self.gerar_posicoes_parede(x1_correia, y1_correia, x2_correia, y2_correia,
                                                      raio_particulas_correia)
        # Parede esquerda
        x1_parede_esq = x2_correia
        ...
        posicoes_parede_esq = self.gerar_posicoes_parede(x1_parede_esq, y1_parede_esq, x2_parede_esq, y2_parede_esq,
                                                         raio_particulas_paredes)
        # Parede direita
        x1_parede_dir = x2_correia + largura_caixa_entrada
        ...
        posicoes_parede_dir = self.gerar_posicoes_parede(x1_parede_dir, y1_parede_dir, x2_parede_dir,
                                                         y2_parede_dir,
                                                         raio_particulas_paredes)
        # Partículas livres
        x1_inlet = x1_parede_esq + raio_particulas_paredes + raio_max_particulas_livres * 1.1
        y1_inlet = raio_max_particulas_livres + raio_particulas_correia
        x2_inlet = x1_parede_dir - raio_particulas_paredes - raio_max_particulas_livres * 1.1
        y2_inlet = y1_inlet
        ...
        # Decompõe a velocidade da correia
        velocidade_correia_x = -math.cos(angulo_real_correia) * velocidade_correia
        velocidade_correia_y = -math.sin(angulo_real_correia) * velocidade_correia

        for i in range(0, len(posicoes_correia)):
            nova_particula = Particula()
            nova_particula.id = self.nova_id()
            nova_particula.raio = raio_particulas_correia
            nova_particula.material = material_correia
            nova_particula.posicao['x'] = posicoes_correia[i]['x']
            nova_particula.posicao['y'] = posicoes_correia[i]['y']
            nova_particula.posicao['angular'] = 0
            nova_particula.livre = False
            nova_particula.grupo = 'correia'

            nova_particula.update_propriedades_fisicas()
            nova_particula.cor = 'blue'

            nova_particula.velocidade['x'] = velocidade_correia_x
            nova_particula.velocidade['y'] = velocidade_correia_y
            # Colocar na lista geral e de fixas
            self.lista_particulas[nova_particula.id] = nova_particula
            self.lista_particulas_fixas.append(nova_particula.id)

        # Definindo a parede esquerda ----------------------------------------------------------------------------------

        for i in range(0, len(posicoes_parede_esq)):
            nova_particula = Particula()
            nova_particula.id = self.nova_id()
            nova_particula.raio = raio_particulas_paredes
            nova_particula.material = material_paredes
            nova_particula.posicao['x'] = posicoes_parede_esq[i]['x']
            nova_particula.posicao['y'] = posicoes_parede_esq[i]['y']
            nova_particula.posicao['angular'] = 0
            nova_particula.livre = False
            nova_particula.grupo = 'parede'

            nova_particula.update_propriedades_fisicas()
            nova_particula.cor = '#B8B8B8'

            # Colocar na lista geral e de fixas
            self.lista_particulas[nova_particula.id] = nova_particula
            self.lista_particulas_fixas.append(nova_particula.id)

        # Definindo a parede direita -----------------------------------------------------------------------------------

        for i in range(0, len(posicoes_parede_dir)):
            nova_particula = Particula()
            nova_particula.id = self.nova_id()
            nova_particula.raio = raio_particulas_paredes
            nova_particula.material = material_paredes
            nova_particula.posicao['x'] = posicoes_parede_dir[i]['x']
            nova_particula.posicao['y'] = posicoes_parede_dir[i]['y']
            nova_particula.posicao['angular'] = 0
            nova_particula.livre = False
            nova_particula.grupo = 'parede'
