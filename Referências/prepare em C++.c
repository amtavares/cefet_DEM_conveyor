...
int nq;
float som;
static double xrp[110000],r[2][110000],rp[2][110000],xr[110000];
...
int main ()
{
  ...
  /* Criando paredes do transportador --------------------------------*/

  float abertura = 2 // em diâmetros
  float distancia_entre_paredes = 30 // em diametros

  float raio_parede = rmin

  float yparticula = 0

  //Parede de trás
  float x_parede_tras = 0
  float y_min_parede_tras = 0
  float altura_parede_tras = 100
  float raios_parede_tras = raio_parede

  for(i=1; i<altura_parede_tras; i++){
    yparticula = y_min_parede_tras + i* 2*raios_parede_tras
    // Cria a partícula na posição
  }

  //Parede da frente
  float x_parede_frente = x_parede_tras + distancia_entre_paredes
  float y_min_parede_frente = abertura * (2*raio_parede)
  float altura_parede_frente = 100
  float raios_parede_frente = raio_parede


  for(i=1; i<altura_parede_frente; i++){
    yparticula = y_min_parede_frente + i* 2*raios_parede_frente
    // Cria a partícula na posição
  }


  // -------------------------------------------------------------------
  // Bloco de partículas livres
